...
REF_FASTA="/home/plstenge/genomes/Ovis_aries.ARS-UI_Ramb_v3.0.dna.toplevel.fa"
BAM_DIR="/home/plstenge/coprolites/08_damage/bwa_n_tests"
OUTDIR_BASE="/home/plstenge/coprolites/08_damage/bwa_n_tests/mapDamage_results"

mkdir -p "$OUTDIR_BASE"

for bamfile in "$BAM_DIR"/aln_n*.bam; do
    bambase=$(basename "$bamfile" .bam)
    outdir="$OUTDIR_BASE/${bambase}_mapDamage"
    
    echo "Running mapDamage on $bamfile output in $outdir"
    mkdir -p "$outdir"
    
    mapDamage -i "$bamfile" -r "$REF_FASTA" --folder "$outdir" 2>>"$outdir/mapDamage.log"
    
    echo "Done $bamfile"
done

echo "All mapDamage runs finished."
